...
hit it. It earned its keep on its first run: a **composite action does not
re-export its steps' outputs**, so both outputs were permanently empty while the
action otherwise looked correct.
